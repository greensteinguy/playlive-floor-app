 ...
 * gap). Payouts come pre-materialized ([{place, amount}]) so this stays pure
 * and the caller keeps its try/catch around materializePayouts.
 */
export function tickerItems(tournament, payouts) {
  ...
  if (paid.length > 0) {
    items.push(`Paying ${paid.length} ${paid.length === 1 ? 'place' : 'places'}`)
    for (const { place, amount } of paid) items.push(`${ordinalPlace(place)} ${formatDisplayMoney(amount)}`)
  }
