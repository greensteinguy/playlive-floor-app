...
// (run-once engine, 10 Aug 2026) — its band rows ("10 – 12") render directly.
// Legacy fallback: the embedded payoutStructure, materialized on the fly. A
// malformed structure must never take down the TV. Every item carries
// {place, toPlace, label, amount} so band-aware consumers (rails, ticker)
// count places, not rows.
function safePayouts(tournament) {
  const rows = tournament?.payoutTable?.rows
  if (rows?.length) {
    return rows.map((r) => ({
      place: r.fromPlace,
      toPlace: r.toPlace,
      label: r.fromPlace === r.toPlace ? ordinalPlace(r.fromPlace) : `${r.fromPlace} – ${r.toPlace}`,
      amount: r.amount,
    }))
  }
