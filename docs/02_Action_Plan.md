...
| 2.6 | ✅ **Live clock engine + TD control screen DONE 30 May 2026.** Server-authoritative but **anchor-based, not per-tick**: each session stores `clockStartIndex` / `clockStartedAt` / `clockPausedAt`, and the live level + countdown are DERIVED from those + the structure durations — so levels **auto-advance at zero with no writes** and every screen stays in sync with **no backend scheduler**. Pure `src/lib/clock.js` (`deriveClock` / `clockState` / `formatRemaining` / `resumeStartedAtMs`, 21 tests); domain ops `src/lib/tournaments/clock.js` (start/pause/resume/advance/rewind/goto/finish — RMW over the session + a denormalized live-state snapshot on the tournament, 23 tests); live `useClock` hook (subscription + 1s tick while running); TD screen at `/td/tournaments/:id/clock` (big blinds/timer, counters, controls gated manager+TD, level-change sound) linked from the detail page. +44 tier-1 tests (378 total); full emulator smoke-test (Start → Advance → Pause → Resume, REST-confirmed anchor + reload re-derive). Auto-advance per Guy (DECISIONS.md 30 May). **TV display is Phase 5** (5.1/5.3, registered-displays model); **tournament-level status transitions are 2.7** (finishClock finishes the session only); multi-flight convergence rollback stays deferred (canonical-schema §5.1). | Working clock | Claude |
| 2.7 | ✅ **Floor controls (tournament status-transition flow) DONE 30 May 2026.** `setTournamentStatus` RMW op drives the lifecycle (`draft → scheduled → lateRegOpen → lateRegClosed → finished`, + cancel from any non-terminal). The **standard sequence is open to manager + TD**; any **non-standard jump** (reopen late reg, un-finish, un-publish) **requires actorRole manager + a `{reason}` override** → emits a `manager.override` audit row beside `tournament.statusChanged`; `finishedAt` stamped/cleared accordingly. Pure transition model (`DEFAULT_TRANSITIONS` + helpers) in `src/lib/tournamentStatus.js`. **Floor controls panel** on the tournament detail page: standard next-status buttons (confirm on finish/cancel) + a manager-only override panel (non-standard targets + reason). +20 tier-1 tests (398 total); full emulator smoke-test (default sequence + a manager override reopening late reg, audit trail confirmed). Status is the tournament-level axis; the session clock (2.6) owns session run-state separately (`finishClock` finishes a session, not the tournament). | Floor controls UI | Claude |
...
| 3.11 | ✅ **Per-player transaction ledger DONE 1 June 2026.** Grew the player-profile **Wallet & tickets** tab from a deposits-only table into the full ledger — every `walletTransaction` type (deposit / spend / ticketUse / winCredit / withdrawal request·complete·cancel / adjustment / openingBalance / managerCredit·Debit), newest-first, with a **running wallet balance** anchored so the latest row matches the cached header balance exactly. Type-filter chips (All / Deposits / Buy-ins / Winnings / Withdrawals / Adjustments, with counts) hide rows without recomputing the balance on the subset; credits green `+`, wallet debits red `−`, no-wallet-effect rows (cash buy-ins, ticket use, withdrawal request/cancel) muted with no sign. CSV export via the shared `downloadCsv`. New pure lib `src/lib/walletLedger.js` (labels + filter groups + `buildLedger` running balance + `summarizeLedger`, 13 tests) reusing the new **`walletTxDelta`** SSOT — extracted into `src/lib/wallet/_shared.js` (handles `adjustment` direction, delegates to `balanceDelta`) and shared with reconciliation so the ledger and `verifyBalanceMatchesLedger` can never drift. No schema change; `useWalletTransactions` already fetched the whole subcollection. `npm test` **615 pass** (+13), lint + build clean; full emulator smoke-test (all 11 types seeded, reconciled end-to-end to a $377.00 header, filters + CSV verified, zero console errors). Functional-first pending 3.12. | Ledger view | Claude |
...
| # | Task | Output | Owner |
|---|---|---|---|
| 4.1 | ✅ **DONE 10 July 2026.** Bust-out recording with **automatic finishing places** — `eliminateEntry` assigns `finishingPlace` = alive count at bust, computed race-safely from fresh in-transaction reads; eliminate confirm previews the place; sole-survivor bust refused (1st only via `recordWinner`, task 4.4); audited `revertElimination` undo (gap-preserving, refused once winnings exist); finishing-order panel on the Players tab. "Remaining stack" deliberately not recorded (a busted player has zero chips). See DECISIONS 10 July §1. | Bust-out UI | Claude |
| 4.2 | ✅ **DONE 10 July 2026.** `drawBounty` — random draw from the undrawn pool, **slot-addressed draw docs** so concurrent draws can't award the same value; skippable eliminator prompt + full-screen reveal + bounty board (remaining count/total) on `/td/tables`; catch-up "Record draw…" path. **Satellite milestone**: `reachSatelliteMilestone` (TD one-tap; no live stack tracking, the TD observes the threshold) marks the entry out via bust fields, `finishingPlace` null, `ticketWinnings` = ticketReward; wallet-side ticket issued at cashier confirm (4.7). Wallet credit for bounties likewise at cashier confirm. DECISIONS 10 July §3–4. | Bounty draw UI + display update + satellite milestone | Claude |
| 4.3 | ✅ **DONE 10 July 2026.** Deck pick (Upper/Main/none) at registration when `hasUpperDeckMainDeck` (best-effort follow-up write after the atomic buy-in); per-deck winner derived from finishing order (one-alive → them; all-busted → unique lowest place; ties/missing → manual); audited settle/unsettle + manual override (`tournament.lastLongerSettled/Unsettled`); panel + roster deck tags on the Players tab. No side-pot ledger (venue handles the money). DECISIONS 10 July §7. | Last-longer logic | Claude |
| 4.4 | ✅ **DONE 10 July 2026.** Per-tournament payouts screen at `/td/tournaments/:id/payouts`: `materializePayouts` (byPercent with the structure's rounding + residue-to-1st so the table sums to the pool exactly; byPlace passthrough) against the real prize pool; `recordWinner` (sole path to 1st; requires exactly one alive) / `revertWinner`. `/td/payouts` nav restored as a thin picker. NB the payout **curve** is still the 2.3 placeholder pending Guy's venue CSV. | Payout calc UI | Claude |
| 4.5 | ✅ **DONE 10 July 2026.** Deal mode on the payouts screen: per-row editable amounts (alive players included), running total vs pool, mismatch requires manager ack + reason (`manager.override`); confirming stages amounts as each entry's `cashWinnings` + `tournament.dealEntered` audit (full agreed table in metadata) — no new collection, per canonical-schema Q4. DECISIONS 10 July §2. | Deal entry UI | Claude |
| 4.6 | ⏸ **PARKED 10 July 2026** per this plan's end-of-Phase-4 decision point — revisit at v1.5 triage. | ICM helper (if time) | Claude |
| 4.7 | ✅ **DONE 10 July 2026.** Per-row cashier confirms on the payouts screen — `confirmEntryWinCredit`: ONE transaction stamps `winningsPaidAt` + `winningsWalletTransactionId` (new nullable Entry fields = the staged-vs-paid split) AND writes the winCredit ledger row AND credits the wallet; idempotent + race-safe, no bulk auto-credit. Per-draw bounty confirms via `confirmBountyWinCredit`; satellite tickets via `confirmEntryTicketWin` (transactional ticket doc + `ticketIssuedAt`/`issuedTicketId` stamps). **Open question for Guy:** wallet credit is the only v1 payout channel — cash-from-till would be a small addition. DECISIONS 10 July §2–3. | Win-credit confirm UI + helper | Claude |
| 4.8 | ✅ **DONE 10 July 2026.** Queue at `/desk/withdrawals` (nav + desk tile restored): cashier/manager create (player picker shared with Deposit via the extracted `PlayerPicker`; UI blocks requests above balance), **manager-only complete** debits the wallet in the same transaction (optional external reference captured at completion — the schema has no notes at creation), cancel needs a reason; filter chips + CSV; `?playerId=` deep link + profile CTA. | Withdrawal queue UI | Claude |
| 4.9 | ✅ **DONE 10 July 2026.** `/admin/reconcile` (cashier + manager): date presets (today default), headline Cash/EFTPOS/PayID-taken + withdrawals-paid cards, money-in/wallet-activity/money-out sections + net wallet change, raw detail table, summary + detail CSVs. Every sign from the `walletTxDelta` SSOT (`getReconciliationTotals` refactored onto the same categorizer). The CG index it needs was deployed 10 July with the collection-group rules fix. DECISIONS 10 July §5. | Reconciliation view | Claude |
| 4.10 | ✅ **DONE 10 July 2026.** `/td/tournaments/:id/results` (all roles, zero writes): grouped standings (placed / ticket winners / still-in-play / busted-no-place) with Paid ✓ / Awaiting-cashier markers, entries + prize pool + guarantee met/missed header, bounty + last-longer + deal-note sections (deal note via the existing audit index, no new index), CSV export. Honest mid-tournament banner. | Results page | Claude |
...
| 5.4 | 🟡 **CODE-LEVEL HALF DONE 10 Aug 2026** — full audit of every operator screen in `docs/reviews/2026-08-10-ipad-pass.md` (23 findings / 14 positives; architecture already touch-first — no drag-drop/hover-gated flows anywhere). All 4 blockers + 4 mechanical majors FIXED (seat pips + money confirms + void/undo + withdrawal actions to ≥44px; phone `inputMode="tel"`; 3 clipped tables now scroll). Remaining: 6 UX-judgment majors (several fold into the seating walkthrough) + 9 minors, listed in the report. **On-device half = Guy's iPad against staging** (checklist at the end of the report) + older-iPad perf check. | iPad pass report | Claude |
