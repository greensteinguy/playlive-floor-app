...
| 2.3 | Payout structure: by position or percentage, with rounding. Auto-generated default, manually overridable. Bounty events get separate bounty-pool field. **Host tab scaffolded 30 May 2026** — the tournament detail page's **Payouts tab** currently renders a read-only positions table (by-percent or by-place); this task replaces it with the editor in place. | Payout structure UI | Claude |
