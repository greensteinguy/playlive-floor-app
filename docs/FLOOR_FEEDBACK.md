...
- [ ] **B1 — Real default tournament templates.** Managers want a standard set: **Turbo, 20-min, 30-min, 40-min, Main Event.** ⛔ **Blocked pending the actual blind structures** (level-by-level SB/BB/ante, durations, breaks) from managers. Deliverable once data is in hand: replace the dev-seed templates with these as the canonical `/td/templates` set (+ seed). *(Alt: draft best-guess defaults now — see assumptions.)*
