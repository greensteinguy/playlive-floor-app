...
**1. Finishing places are assigned by the bust, computed in-transaction (4.1).** `eliminateEntry` sets `finishingPlace` = the number of alive entries at the moment of the bust, computed from **fresh reads of every entry inside the transaction** (Firestore web-SDK transactions can't run queries, so entry ids are listed pre-transaction and every doc re-read inside; concurrent eliminations serialize on the shared read set, so duplicate places are impossible). Known accepted hole: an entry *created* between the listing and the commit isn't in the read set — a registration landing in the same instant as a bust could skew one place by one. **Eliminating the sole survivor is refused** (`LastPlayerStandingError`) — 1st place is only ever assigned by `recordWinner` on the payouts screen, so a bust can never crown a winner. **Reverting an elimination does not renumber later finishers** (the gap is honest floor history), and the revert refuses once the target or any later-busted entry has recorded winnings (`RevertBlockedByPayoutError`).

**2. Staged vs paid winnings are two different fields (4.4/4.5/4.7).** `cashWinnings` on the entry = the *recorded result* (set by a confirmed calculator payout or a deal; arms the revert guard). **`winningsPaidAt`** (new nullable field, + `winningsWalletTransactionId` for two-way ledger trace) = the *cashier-confirmed payment marker*, stamped in the SAME transaction that writes the winCredit ledger row and credits the wallet. Survives reloads, race-safe, one confirm per row — no bulk auto-credit anywhere, per the SOW. A deal is exactly the schema's design: per-entry `cashWinnings` overrides + a `tournament.dealEntered` audit row (metadata carries the full agreed table + previous amounts); a deal total ≠ prize pool needs a manager acknowledgment + reason and additionally emits `manager.override`. Rounding residue from the payout curve is dumped on 1st so the table always sums to the pool exactly — but only when the percent structure itself sums to ~100%. **Open question for Guy:** wallet credit is the only v1 payout channel; if winners routinely take till cash instead, a "paid in cash" channel is a small addition the schema already leaves room for.

**3. Satellite milestone exits reuse the bust fields but keep `finishingPlace` null (4.2).** `reachSatelliteMilestone` marks the entry with `bustedAt`/seat-cleared exactly like a bust — so every alive-count rule (`computeEntryCounters`, place assignment, waitlist) sees them as out of the field with zero forked logic — but `finishingPlace` stays null (canonical §5.2: a milestone exit is a win, not a place) and `ticketWinnings` = `satelliteConfig.ticketReward` marks them a winner. The wallet-side ticket is issued at cashier confirm (`confirmEntryTicketWin`, with `ticketIssuedAt` + `issuedTicketId` entry stamps — enforced both-or-neither since issuance and the ticket doc are one transaction). No milestone undo op in v1 (a mis-tap is a manual correction; `revertElimination` deliberately refuses milestone entries via the winnings guard).

**4. Mystery bounty draws are slot-addressed for race safety (4.2).** Each published pool value maps to a deterministic `bountyDraws/slot-{i}` doc id; the draw transaction probes its chosen slot, so two simultaneous draws of the same value collide on the doc and one retries with a re-pick. The eliminator prompt is always skippable (never blocks recording the elimination) with a catch-up "Record draw…" path. Wallet credit again waits for cashier confirm.

**5. Reconciliation reuses the wallet-delta SSOT and needed the app's second collection-group index (4.9).** The report categorizer is pure and every sign comes from `walletTxDelta` — unknown types throw loudly rather than silently dropping money. Buy-ins paid by wallet/ticket are *excluded* from external money-in (that money entered on a prior day); withdrawals are a separate money-OUT headline (their payout channel isn't recorded per-method on the schema). `firestore.indexes.json` gained a `walletTransactions.timestamp` fieldOverride **including the COLLECTION-scope entries** (an override *replaces* the automatic single-field indexes, and the per-player ledger depends on them — omitting them would have broken the profile Wallet tab). **Deploy note:** the parallel CG-rules session (same day, HANDOFF 10 July) folded this override into its collection-group fix (superset: + CONTAINS + CG ASC) and **deployed it to production alongside the CG read rules** — so the reconciliation page's production query needs are already live.

**6. Withdrawal notes live at completion, not creation (4.8).** The schema has no notes field on a request and `externalReference` belongs to completion — so the create flow captures player/amount/method only and the manager's complete-confirm takes the optional reference. UI blocks creating a request above the current balance; the hard `walletBalance ≥ 0` invariant still fires at completion.

**7. Last-longer deck capture is a best-effort follow-up write after the buy-in (4.3).** The wallet payment ops hardcode `lastLongerDeck: null` on fresh entries, so the deck pick is applied in a second validated write after the atomic payment — deliberately best-effort, because a failure after a committed buy-in must NOT read as a failed registration (the desk would re-charge); staff get an explicit warning toast instead. Winner derivation refuses to guess: one alive → them; all busted → unique lowest place; ties/missing places → manual, audited override pick (which is also the correction path for out-of-order busts).

**8. Results page is derived-only (4.10).** Zero writes; standings from `finishingPlace` + `recordedWinner`; honest mid-tournament ("Still in play" group + banner). The deal note reuses the existing `(targetType, targetId, timestamp)` audit index with client-side actionType filtering — no new index. Known coarse edge: a bounty-only earner can show "Awaiting cashier" on the standings row even after per-draw credits (the bounty section below is per-draw accurate; payouts screen is operational truth).

**Verification:** every branch green on tests/lint/build before merge; merged `main` 817 pass, lint + build clean; full emulator browser smoke-test of the core chain PASSED (see HANDOFF 10 July). Bounty/satellite/last-longer/deal flows are unit-tested but not yet browser-driven — fold into Guy's Phase-4 review.

**Decider:** Guy (the phase + parallel-agent approach); the per-item calls above were Claude's, flagged here for ratification.

## 4 June 2026 — Seating room-overview rework + player elimination

Guy's flagged seating-layout rework (3 June backlog) plus a new floor action — eliminating a knocked-out player. Both on `feature/seating-room-overview-eliminate` (branched off `feature/ui-futuristic-reskin`, not merged).

**1. The `/td/tables` room is now a dense at-a-glance overview; full seat grids are drill-down only.** The page used to stack every table's full 9-row seat list down the page — unusable with a real field of many tables (Guy: "doesn't scale to many tables"). Now each table is a **compact card** leading with a one-line **✓/✗ seat map** (✓ = taken, ✗ = open, seat number on each) — the spatial occupancy read at a glance — with the `N/M` count in the header. The **named seat list** (who's in each seat) is revealed per-card via **"Show seats"**, or all at once via **"Expand all"**. **✓/✗ over filled/empty pips** was Guy's pick (offered both; he wanted the explicit "full vs empty" semantic). The compact pips are fully interactive — clicking a ✓ picks that player up, clicking a ✗ drops the held player — so the move workflow doesn't require expanding; expand is for reading names. No seat/table data or atomic-op change — purely the page's layout + a per-table expand state.

**2. Player elimination (bust-out) is pulled forward from Phase 4 as a floor action, but WITHOUT finishing places.** Guy wanted "a way to eliminate players… like an option when picking up a player off a table." New atomic op **`eliminateEntry`** (in `seating.js`, beside `unseatEntry`): frees the seat (if seated) and marks the entry busted — sets `bustedAt` + `bustedInSessionId` and clears the seat fields **in one write** so the entry's bust/seat invariants hold — then a **best-effort `recountTournamentEntries`** so the tournament's `remainingPlayerCount` drops. Audited `entry.busted` (the action type already existed). Surfaced in the picked-up-player banner as **`eliminate`** (next to `unseat`) behind an inline confirm.
   - **`finishingPlace` is left null** — Guy chose "just mark eliminated" over auto-recording a place. The roster shows **"Busted"** (the existing `entryResultLabel` already handles this). **Finishing order + payouts stay Phase 4.** Rationale: keeps this a clean seating/floor action and doesn't bake half of the Phase-4 payout model in early; when Phase 4 lands it owns place assignment end-to-end. (`finishingPlace` and the bust fields are forward-compatible — Phase 4's bust path can set the place; this op just doesn't.)
   - **Distinct from `unseatEntry`:** unseat returns a player to the alternates pool (still seatable); eliminate makes them un-seatable and drops them off the waitlist + the field.
   - **`seating.js` now imports `recountTournamentEntries` from `./registration`** (one-directional, no cycle — registration doesn't import seating), mirroring how `registerEntry` recounts internally: any op that changes the live-entry set refreshes the cached counters.

**Flagged for the floor-staff seating walkthrough** (with the rest of the functional-first seating UI — see [[seating-ux-pending-floor-review]]): no un-eliminate UI in v1 (a mis-click is fixed by re-seating/re-entry or a manual data correction); `bustedInSessionId` is taken as the active session (right for single-day; revisit for a multi-day entry busting in a later session); a simultaneous double-elimination on the same tournament could momentarily share counter state (single-TD-per-tournament in practice — accepted).

**Verification:** `npm test` **619 pass** (+4 in `seating.test.js`: eliminate a seated entry frees the seat + sets bust fields + recounts + audits; eliminate an unseated entry with no table write; refuse an already-busted entry; refuse a voided entry — the recount is stubbed via a `./registration` mock), lint + build clean. Full emulator browser smoke-test PASSED (Thursday $80 Turbo, manager role): the room rendered as compact ✓/✗ cards (T1 `1✓2✓3✓4✗…`, T3 all ✗) with Expand-all + per-table Show-seats; expanding T1 showed the named list (Charlie Davis / Bob Solo / Chuck Davis); picking Charlie up via his ✓ pip showed "Holding Charlie Davis — … unseat · eliminate"; eliminate → the confirm → execute dropped the field **5→4**, freed his seat (T1 3→2), showed "1 out", and cleared the banner. **REST-verified** the entry (`bustedAt` set, `bustedInSessionId` = the day-1 session, `finishingPlace` **null**, seat fields null, `voidedAt` null), T1 seat 1 empty, tournament `entryCount` 5 / `remainingPlayerCount` 4, and the `entry.busted` audit row (actorRole manager, metadata.sessionId). Console clean. (Screenshots couldn't be captured — the reskin's animated background never reaches the screenshot tool's "visually stable" gate; verified via the accessibility snapshot + REST instead.) **Dev-emulator residue:** Charlie Davis (`player-charlie-1`) is now busted on Thursday Turbo; a re-seed resets it.

**Decider:** Guy (the rework + the eliminate feature; the ✓/✗ choice and the no-finishing-place choice via the kickoff questions). The compact-pip-is-interactive design, the `eliminateEntry` shape, recount-in-op, and the deferrals above were Claude's implementation calls.
