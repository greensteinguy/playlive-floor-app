...
- **Multi-flight (funnel, N→N→1):** flights funnel toward the single final session, stage by stage. A stage may have several flights; each flight feeds **exactly one** flight in the next stage (fan-in only, no fan-out), and a stage's flights **partition** the previous stage's flights ("survivors from"). Simplest case: Day 1A + Day 1B both `convergesIntoSessionId` → one Day 2 (slice picks up at [15..]). Deeper case: Day 1A–1D → Day 2ABC + Day 2DEF (grouped by origin) → Day 3, WSOP-style. Flights of the same stage share a maximum slice; `buildSessionDocs` wires the graph from the routing partition (`src/lib/tournaments/sessions.js`). The final session has `maximumEndIndex=null` (play to a winner) unless capped.
