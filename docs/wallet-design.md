...
**Hard invariant on wallet balance:** `wallets/{playerId}.balance` must always be ≥ 0. Any `spend` of `method = "wallet"` that would push balance negative is rejected at the wallet module layer **and** at the Firestore rules layer (defence in depth). There is no manager-override path — if a player has insufficient wallet balance, the cashier takes a deposit or alternative payment first.
...
| Q6 | **Wallets can never go negative. Hard rule, no manager override.** | Spend that would push balance < 0 is rejected at the wallet module layer (Phase 1 task 1.7) and again at the Firestore rules layer (Phase 1 task 1.2). No UI override path. Cashier handling the case must take a top-up via another method first. |
