...
- `npm test` → 297 tier-1 unit tests, ~2.4s.
...
## This session (30 May 2026) — tournament detail page scaffold
...
1. **Continue Phase 2: ~~2.5 (templates) ✅~~ → ~~2.1 (tournament create) ✅~~ → ~~2.2 (list) ✅~~ → ~~tournament detail scaffold ✅~~ → 2.4 (multi-format wizards) or 2.3 (payout editor) NEXT → 2.3 (clock) → 2.6 (seating) → 2.7 (status transitions).** Task 2.4 is the sessions model — the trickiest part of the project so far; re-read canonical-schema.md §5.1 (the `convergesIntoSessionId` routing graph + planned-vs-actual slice indices). The detail page (30 May) delivered the **Details + Structure edit mode** 2.1 had deferred; the remaining 2.1 core-field edits (fee/guarantee are already on the Details form) + multi-day/multi-flight/satellite *setup* roll into 2.4. **The Payouts tab is now scaffolded as a read-only view — its editor is task 2.3 and drops straight into that tab.** (The live clock is an alternative next step if Guy wants to see a tournament actually run sooner — it only needs `subscribeToTournament`, which already exists, and the detail page is its host.)
...
```
npm test               # tier 1: validators + wallet + tournaments (297 tests, ~2.4s)
